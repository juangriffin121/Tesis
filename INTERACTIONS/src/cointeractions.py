...
names = [
    ...
    "L1CAM",
    "LAMP1",
]  # motif-containing interacting prots
...
for App_interacting_prot in App_dict:
    for name in names:
        for Anot_int in Anotated_interactions_dict[name]:
            if (
                App_dict[App_interacting_prot]["id"]
                == Anotated_interactions_dict[name][Anot_int]["id"]
            ):
                cointeractions.append(
                    App_interacting_prot
                    + f" interacciona con {name} ademas que con App"
                )  # App_dict[App_interacting_prot]['id']
...
# Interaccionan con las otras prots
